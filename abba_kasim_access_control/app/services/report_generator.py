...
from app.models import db, AuditSession, AuditFinding, Role, User, Permission

class AuditReportGenerator:

    @classmethod
    def generate_pdf_report(cls, audit_session_id, output_path):
        """Generates a professional ReportLab PDF Access Control Audit Executive Report."""
        ...
        story.append(Paragraph(f"<b>Student Project:</b> Abba Kasim &nbsp;|&nbsp; <b>Execution Date:</b> {session.created_at.strftime('%Y-%m-%d %H:%M:%S UTC')} &nbsp;|&nbsp; <b>Auditor:</b> {session.executed_by}", subtitle_style))
        story.append(HRFlowable(width="100%", thickness=1.5, color=colors.HexColor('#2563EB'), spaceAfter=15))

        # Executive Metrics Summary Cards Table
        metric_score = f"<font size=16 color='#2563EB'><b>{session.least_privilege_score}%</b></font><br/><font size=8 color='#64748B'>LEAST PRIVILEGE SCORE</font>"
        ...
        exec_text = f"An automated Access Control Audit was conducted to evaluate compliance with the <b>Principle of Least Privilege (PoLP)</b>, NIST SP 800-53 Access Control standards, and Separation of Duties (SoD) guidelines. The evaluation identified <b>{len(findings)} security findings</b> across the organization's role and user access matrix."
        ...
        # Section 3: Actionable Hardening Recommendations
        ...
        sign_text = "<b>Report Certification:</b> Generated by Abba Kasim's Access Control Audit Platform. Verified for Least Privilege Compliance."
    ...
    def generate_csv_report(cls, audit_session_id, output_path):
        """Generates a CSV export of audit findings."""
        session = AuditSession.query.get(audit_session_id)
        if not session:
            raise ValueError(f"AuditSession #{audit_session_id} not found.")

...
class AccessControlAuditEngine:
    # Toxic Permission Pairs (Separation of Duties - SoD Violations)
    SOD_TOXIC_PAIRS = [
        {
        ...
        {
            "codes": ("exec:system_backup", "delete:audit_logs"),
            "title": "Audit Tampering & Anti-Forensics Risk",
            "severity": "HIGH",
            "risk": "Role can perform system maintenance and delete audit trails, enabling malicious actors to cover post-exploitation activity.",
            "remediation": "Revoke audit log deletion permissions. Forward audit logs to an immutable write-once read-many (WORM) central log repository."
        },
        {
    ...
    @classmethod
    def run_full_audit(cls, executed_by="System Auditor", session_title="Least Privilege Compliance Audit"):
        """Performs a comprehensive access control audit across all roles, permissions, and users."""
        ...
        # 1. Evaluate Roles for Over-Privilege and High-Risk Accumulation
        for role in roles:
            ...
            # Rule A: Excessive High-Risk Permissions
            ...
            # Rule B: Non-Technical Role with Root/Admin Privileges
            if role.department.lower() not in ['it security & compliance', 'system administration', 'it infrastructure']:
                for p in role.permissions:
                    if p.code in ['exec:root_sudo', 'delete:audit_logs', 'manage:firewall']:
                        overprivileged_roles_count += 1
                        findings.append({
            ...
            # Rule C: Separation of Duties (SoD) Conflicts within Role
        ...
        # 2. Evaluate Users for Dormant Admin Accounts and User-Level SoD
        for user in users:
            user_perm_codes = set()
            ...
            # Rule D: Dormant Admin / Over-Privileged User
            ...
            # Rule E: Multi-Role Toxic Combination (User holds multiple roles that combined create SoD conflict)
        ...
        # 3. Compute Least Privilege Score (LPS)
        ...
        # 4. Save Audit Session & Findings to Database
        ...
        db.session.flush() # get audit_session.id

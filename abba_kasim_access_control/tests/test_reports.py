...
def test_report_generation(app):
    with app.app_context():
        ...
        pdf_path = os.path.join(app.config['REPORTS_FOLDER'], f'test_report_{session.id}.pdf')
        csv_path = os.path.join(app.config['REPORTS_FOLDER'], f'test_report_{session.id}.csv')

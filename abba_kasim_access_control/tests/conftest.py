...
def app():
    ...
    test_db_path = os.path.join(os.path.dirname(__file__), 'test_audit.db').replace('\\', '/')
    
    app = create_app()
    app.config['TESTING'] = True
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{test_db_path}'
    app.config['WTF_CSRF_ENABLED'] = False
    app.config['SECRET_KEY'] = 'test-secret-key-abba-kasim'

    with app.app_context():
        db.create_all()
        
        # Seed test permissions if missing
        ...
        if not User.query.filter_by(username='test_admin').first():
            admin = User(username='test_admin', email='admin@test.local', full_name='Test Admin', role='ADMIN')
